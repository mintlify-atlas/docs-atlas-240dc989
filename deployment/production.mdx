...
```
Checkout
  → Configure AWS credentials (OIDC-style via aws-actions/configure-aws-credentials@v5)
  → Login to Amazon ECR (aws-actions/amazon-ecr-login@v2)
  → Install uv + serverless@3 + serverless-domain-manager
...
| `AWS_ACCOUNT_ID` | AWS account ID (used by ECR login) |
...
npx serverless deploy function -f nightlyBkt --stage prod
npx serverless deploy function -f llmClassifier --stage prod
```
